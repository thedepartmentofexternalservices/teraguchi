...
# This test does not change the product deployment target. A local test may
# explicitly select the target used by its prepared Qt/SDL dependencies.
...
for scenario in idle streaming disconnect unrelated queued-quit duplicate-quit after-sdl-shutdown repeated-disconnect; do
