...
plank_macos_client_dependency_profile() {
    local mode=$1 source_root=$2 profile expected patch_hash
    profile="$PLANK_MAC_CLIENT_DEPS/.teraguchi-client-profile"
    patch_hash=$(shasum -a 256 "$source_root/scripts/build/ffmpeg-patches/0002-videotoolbox-require-and-attest-hardware.patch") || return
    expected=$(printf 'target=%s\nsdk=%s\nhardware_patch=%s\n' \
        "$PLANK_MACOS_CLIENT_TARGET" "$PLANK_MACOS_CLIENT_SDK" "${patch_hash%% *}")
    if [[ -f $profile ]]; then
