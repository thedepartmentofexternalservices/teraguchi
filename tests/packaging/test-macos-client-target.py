...
class ClientTarget(unittest.TestCase):
    ...
    def test_changed_bootstrap_recipe_rejects_cached_libraries(self):
        source=self.base/'source'
        (source/'scripts/build').mkdir(parents=True)
        recipe=source/'scripts/build/bootstrap-macos-client-deps.sh'
        shutil.copyfile(ROOT/'scripts/build/bootstrap-macos-client-deps.sh',recipe)
        tail='plank_macos_client_dependency_profile bootstrap "$TEST_SOURCE_ROOT"'
        self.assertEqual(self.run_helper(tail,TEST_SOURCE_ROOT=str(source)).returncode,0)
        recipe.write_text(recipe.read_text()+'\n# changed dependency recipe\n')
        tail='plank_macos_client_dependency_profile build "$TEST_SOURCE_ROOT"'
        self.assertNotEqual(self.run_helper(tail,TEST_SOURCE_ROOT=str(source)).returncode,0)

    def test_invalid_profile_operation_rejected(self):
        self.assertNotEqual(self.run_helper('plank_macos_client_dependency_profile typo "$2"').returncode,0)

    def test_missing_profile_rejected_for_build(self):
        self.assertNotEqual(self.run_helper('plank_macos_client_dependency_profile build "$2"').returncode,0)

    def test_malformed_sdk_rejected(self):
        for sdk in ['', 'unknown', '26x', '26.0beta']:
            with self.subTest(sdk=sdk):
                self.assertNotEqual(self.run_helper(TEST_SDK=sdk).returncode,0)
